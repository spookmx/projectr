...
Meteor.methods({
  ...
  addRequest(request){
    ...
    request.viewRequester = null;
    request.viewRepresentative = null;
    //Add contact to Rep
...
<<<<<<< HEAD
  rateRepresentative(ratingInfo){
    return Requests.update(
      { _id: ratingInfo.requestId },
      { $set: { rating: ratingInfo.rating } }
    );
=======
  removeContact(contactId){
    if (!this.userId) {
      throw new Meteor.Error('not-logged-in', 'Must be logged in to remove contacts.');
    }
  ...
  updateRequestView(requestId){
    if (!this.userId) {
      throw new Meteor.Error('not-logged-in', 'Must be logged in to update request view date.');
    }
    let request = Requests.findOne({ _id: requestId });
    if(request.requester = this.userId){
      return Requests.update(
        { _id: requestId },
        { $set: { viewRequester: new Date() } }
      );
    }else{
      return Requests.update(
        { _id: requestId },
        { $set: { viewRepresentative: new Date() } }
      );
    }
>>>>>>> origin/master
  }
