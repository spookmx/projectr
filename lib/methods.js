...
Meteor.methods({
  ...
  addRequest(request){
    if (!this.userId) {
      throw new Meteor.Error('not-logged-in', 'Must be logged in to create requests.');
    }
  ...
  cancelRequest(cancelInformation){
    if (!this.userId) {
      throw new Meteor.Error('not-logged-in', 'Must be logged in to cancel a request.');
    }
